    ...
    if (found) {
        cart.forEach(element => {
            if (element.id == found.id) {
                element.quantity++
                document.getElementById("prodQ-"+element.id).innerHTML = element.quantity
            }
        });
    } else {
        ...
        productsCheckoutHtml += `
                            <tr id ="prod-${currentProduct.id}">
                                ...
                                <td id="prodQ-${currentProduct.id}">${currentProduct.quantity}</td>
                                <td>${currentProduct.price}</td>
                                <td>
                                    <div onclick="remove (${currentProduct.id})" class="btn btn-danger">Delete</div>
                                </td>
                            </tr>`
        document.getElementById("quantite").innerHTML = cart.length;
...
function remove  (index)  {
    cart = cart.filter (element => element.id != index)
    document.getElementById(`prod-${index}`).remove();
    document.getElementById("quantite").innerHTML = cart.length;
}